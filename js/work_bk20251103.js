...
document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    works.forEach((work, index) => {
      ...
      imgTag.src = work.mainImage;
      imgTag.alt = work.title;
      ...
      textDiv.innerHTML = `
        <h2>${work.title}</h2>
        <div class="desc-block">${descHTML}</div>
        ${
          work.link
            ? `<p class="link"><a href="${work.link}" target="_blank" rel="noopener noreferrer">サイトを見る</a></p>`
            : ""
        }
      `;
